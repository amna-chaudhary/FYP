/* =============================================================
   certificate-view.js
   Renders the printable Granular Energy Certificate.
   This module is a 1:1 port of the canonical render layer at
     apps/Gec_Server_C/frontend/static/certificate-view.js
   so the chat app and the canonical app share the same design.

   Public API:
     window.normalizeCertificate(payload) -> normalized data
     window.renderCertificate(container, payload) -> renders into container
   ============================================================= */
...
(function () {
  ...
  // render in UTC rather than the viewer's local timezone unless the input
  // timestamp does not include any zone information.
  ...
  function formatCertId(rawId, createdAt, txHash) {
    if (rawId) {
      const text = String(rawId);
      if (/^GEC-\d{4}-\d{4,}$/i.test(text)) return text.toUpperCase();
      const numericMatch = text.match(/(\d+)/);
      if (numericMatch) {
        const padded = numericMatch[1].padStart(6, "0");
        const year = (parseDate(createdAt) || new Date()).getFullYear();
        return `GEC-${year}-${padded}`;
      }
    }
    // Fallback: derive a stable display id from the tx hash so chat-only
    // payloads (no backend cert_id yet) still render a real-looking ID.
    if (txHash && typeof txHash === "string") {
      const hex = txHash.replace(/^0x/i, "");
      const tail = hex.slice(-6);
      const num = parseInt(tail, 16);
      const year = (parseDate(createdAt) || new Date()).getFullYear();
      if (Number.isFinite(num)) {
        return `GEC-${year}-${String(num % 1000000).padStart(6, "0")}`;
      }
    }
    return "—";
  }
  ...
  function normalizeCertificate(payload) {
    ...
    const energyAmount = pick(
      cert.energy_amount,
      cert.energyAmount,
      cert.energyGenerated,
      cert.issued_quantity,
      cert.issuedQuantity,
      cert.quantity,
    );

    const ownerAccount = pick(
      cert.owner_account_address,
      cert.ownerAccount,
      cert.owner,
      cert.owner_account_id,
      cert.ownerAccountId,
      cert.account_id,
      cert.accountId,
      cert.sender_address,
    ...
    const previousOwner = pick(
      ...
      cert.previousOwner,
      cert.source_account_id,
      cert.sourceAccountId,
      cert.previousOwner,
    );
    ...
    return {
      rawId: cert.id,
      displayId: formatCertId(pick(cert.display_id, cert.displayId, cert.cert_id, cert.certId, cert.id), createdAt, txHash),
      status: rawStatus,
